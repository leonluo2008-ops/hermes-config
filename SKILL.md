...
description: |
  Hermes 配置专员——本机唯一负责"Hermes 怎么配置"的 skill。整合了 model/provider/delegation/profile/gateway/toolset/平台诊断/配置分层/provider 管理/斜杠命令/.hermes.md 初始化等知识。
  当用户需要配置 Hermes 本身（不是用 Hermes 做事，而是配置 Hermes 这个系统）时加载。
  触发词：Hermes 配置专员、配置专员、让配置专员看一下、让配置专员检查、hermes 配置、检查 hermes 配置、配置健康检查、改 model、切 provider、delegation model、子 agent 模型、跨模型、fallback、profile 管理、gateway 重启、toolset 加载、token 无效、config 改了不生效、飞书 doc 读不到、frozen session、MoA schema、聚鑫 base_url、vision 模型切换、斜杠命令、.hermes.md 初始化、SOUL.md 放什么、配置分层、providers.json、API key 管理、这条规则放哪、配置怎么组织。
...
| 场景 | 历史主模型 | 子 agent 模型 | 方式 | 跨模型？ |
|---|---|---|---|---|
| 对抗审查 (plan_review) | M3（→ GLM-5.2） | GLM-5.2（用户刻意指定） | delegate_task + delegation.model 全局切 | ✅ 需手动确认 ≠ 主力 |
| 辅助决策 (decision_support) | GLM-5.2 | gemini-3.5-flash | 独立脚本调 API | ✅ 精确指定 |
| 代码验证 (code_review) | GLM-5.2 | 继承（验证靠跑命令不靠跨模型） | delegate_task spawn | ❌ 不需要跨模型 |
| course-builder 写作评估 | gemini-3.5-flash | gemini-2.5-flash | call_llm_step.py | ✅ 精确指定 |

**铁律（2026-07-24 用户纠正）**：plan_review 的设计意图是**子 agent 用跟主力不同的模型**做跨模型审查。
- 最初主力 M3 → 用户指定 plan_review 用 GLM-5.2 → 跨模型成立
- 后主力换 GLM-5.2 → 如果 delegation.model 没改，子 agent 继承 GLM-5.2 → 跨模型**失效**
- **主力模型会变，跨模型的设计意图不变。每次用 plan_review 前确认 `hermes config get delegation.model` ≠ 当前主力模型。**
- 详见 `qa-test-agent/references/delegation-model-selection.md`（3 种方式 + 实战案例）
